...
class ActaURL(Enum):
    UNO = "https://preliminar.tse.gob.sv/administracion/img/get-acta"
    DOS = "https://divulgacion.tse.gob.sv/administracion/img/get-acta-dos"
...
class Acta:
    ...
    def __str__(self):
        return f"URL: {self.url}, STATUS: {self.status}, UPLOADED: {self.uploaded}, DATETIME: {self.datetime}, FILE_NAME: {self.file_name}, HASH: {self.hash}"  # noqa: E501

    def __repr__(self):
        return f"{self.url}, {self.status}, {self.uploaded}, {self.datetime}, {self.file_name}, {self.hash}"  # noqa: E501


class DataSources:

    columns = ["URL", "STATUS", "UPLOADED", "DATETIME", "FILE_NAME", "HASH"]
...
# Create function to initialize the dataframe with the URLs of the actas from 1 to total
@logger.catch
def init_data_sources(TOTAL, BASE_URL, data_sources, START=1):
    logger.info("Initializing data_sources ...")
    for number in range(START, TOTAL + 1):
        data_sources.add_acta(Acta(f"{BASE_URL}/{number}"))
    logger.info("data_sources initialized, OK")
    return data_sources
...
def download_acta(acta):
    ...
    try:
        # URL Request
        response = requests.get(acta.url)
        # If the status code is 200
        if response.status_code == 200:
            # If the response content is not empty
            if response.content:
                acta.hash = hashlib.sha256(response.content).hexdigest()
                acta.file_name = f"{acta.hash}.jpeg"
                # If the file does exist
                if os.path.exists(f"src/data/0_raw/{acta.file_name}"):
                    acta.file_name = f"{acta.hash}_{acta.datetime}.jpeg"
                # Save the acta file
                with open(f"src/data/0_raw/{acta.file_name}", "wb") as f:
...
def upload_acta_to_s3(acta):
    try:
        ...
        S3_CLIENT.upload_file(
            f"src/data/0_raw/{acta.file_name}",
            BUCKET_NAME,
            acta.file_name,
            Callback=ProgressPercentageUploadToS3(f"src/data/0_raw/{acta.file_name}"),
        )
        acta.uploaded = True
...
def process_data_sources(data_sources, chunk_size=100):
    ...
    with Pool(num_processes) as pool:
        ...
        # results = pool.imap_unordered(process_chunk, enumerate(chunks))
        # results = list(results)
        # results = pool.map_async(process_chunk, enumerate(chunks))
        # results.wait()
        # results = results.get()
...
def febrero(
    ...
    logger.info("Running febrero ...")

    # Create a febrero DataSources
    ...
    if datasources_file:
        ...
        # Initialize the dataframe with the URLs of the actas from 1 to 10000
        init_data_sources(TOTAL_UNO, ActaURL.UNO.value, data_sources, START_UNO)

        # Initialize the dataframe with the URLs of the actas from 1 to 29000
        init_data_sources(TOTAL_DOS, ActaURL.DOS.value, data_sources, START_DOS)
    ...
    # Get the total actas downloaded
    ...
    # Get the total actas uploaded to S3
    ...
    # Get dataframe with the actas
    df_actas = data_sources.to_df()
    # Logger Total Actas Done from df_actas
    logger.info(
        f"Total Actas Done from df_actas: {df_actas[df_actas['STATUS'] == 'done'].shape[0]}"  # noqa: E501
    )

    # Select the actas with hash not null
    df_actas_not_null = df_actas[df_actas["HASH"].notnull()]
    # Get the total actas duplicated
    total_actas_duplicated = df_actas_not_null[
        df_actas_not_null.duplicated(subset="HASH")
    ].shape[0]

    # Logger Total Actas Duplicated from df_actas_not_null
    logger.info(
        f"Total Actas Duplicated from df_actas_not_null: {total_actas_duplicated}"  # noqa: E501
    )

    logger.info("febrero finished, OK")
    ...
    ds_file_name = f"febrero_{end_datetime}.csv"
    data_sources.save(f"src/data/{ds_file_name}")

    # Save actas duplicated from df_actas_not_null to csv
    df_actas_not_null[df_actas_not_null.duplicated(subset="HASH")].to_csv(
        f"src/data/febrero_duplicated_{end_datetime}.csv", index=False
    )
